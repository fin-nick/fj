...
public final class SrvResolver {
     ...
     */
    public SrvResolver(String dnsServer) {
        server = dnsServer;
    }
    ...
    public String get(String[] domain) {
        try {
            return sendUdp(packet(domain));
        } catch (IOException ex) {
            return null;
        }
    }

    private String sendUdp(byte[] message) {
        DatagramConnection dc = null;
        String result = null;    
        try {
            dc = (DatagramConnection)Connector.open("datagram://" + server + ":53");
            dc.send(dc.newDatagram(message, message.length));
            Datagram dobject = dc.newDatagram(512);
            dc.receive(dobject);
            result = read(dobject.getData());
        } catch (Exception e) {
        }
        if (null != dc) {
            try {
